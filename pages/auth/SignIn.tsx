...
import { Mail, Lock, Check, CheckCircle, Eye, EyeOff, X } from 'lucide-react';
...
export const SignIn: React.FC = () => {
  ...
  const [rememberMe, setRememberMe] = useState(() => {
    return localStorage.getItem('neonotex_remember_me') === 'true';
  });
  ...
  const prefilledEmail = location.state?.email || (rememberMe ? localStorage.getItem('neonotex_saved_email') : '') || '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        ...
        if (rememberMe) {
          localStorage.setItem('neonotex_remember_me', 'true');
          localStorage.setItem('neonotex_saved_email', email);
        } else {
          localStorage.removeItem('neonotex_remember_me');
          localStorage.removeItem('neonotex_saved_email');
        }
  ...
  return (
    <div className={`relative flex min-h-screen w-full items-center justify-center overflow-hidden font-sans selection:bg-slate-950/20 ${activeTheme === 'midnight-black' ? 'bg-[#000000]' : activeTheme === 'sakura-pink' ? 'bg-[#FFF5F8]' : 'bg-[#F4F6F9]'}`}>
      ...
      <div className="relative w-full max-w-[440px] px-6 transition-all duration-700 ease-out hover:-translate-y-2">
        ...
        <div className={`group relative overflow-hidden rounded-[36px] border p-8 sm:p-10 shadow-[0_30px_60px_-10px_rgba(0,0,0,0.08),0_10px_20px_-5px_rgba(0,0,0,0.04)] backdrop-blur-[50px] transition-all duration-500 hover:shadow-[0_45px_80px_-12px_rgba(0,0,0,0.12),0_15px_30px_-5px_rgba(0,0,0,0.06)] ${activeTheme === 'midnight-black' ? 'border-white/10 bg-[#161616]/80 hover:bg-[#1C1C1E]' : 'border-white/60 bg-white/40 hover:bg-white/50'}`}>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className={`mt-2.5 text-[13px] font-medium text-center max-w-[280px] leading-relaxed ${activeTheme === 'midnight-black' ? 'text-slate-400' : 'text-slate-500'}`}>
              Your thoughts deserve privacy. Neonotex keeps your notes secure with end-to-end encryption.
            </p>
            ...
            <form onSubmit={handleSubmit} className="mt-10 w-full space-y-6">
              ...
              <div className="flex items-center justify-between px-1">
                  <label className="flex items-center gap-2.5 cursor-pointer group/chk select-none">
                      <div className={`relative flex items-center justify-center w-5 h-5 rounded-[6px] border shadow-sm transition-all ${activeTheme === 'midnight-black' ? 'border-white/20 bg-[#161616] group-hover/chk:border-[#F7C948]' : 'border-slate-300 bg-white/50 group-hover/chk:border-slate-800'} group-hover/chk:shadow-md`}>
                        <input 
                          type="checkbox" 
                          name="rememberMe"
                          checked={rememberMe}
                        ...
                        />
                        <Check size={12} className={`${activeTheme === 'midnight-black' ? 'text-[#F7C948]' : 'text-slate-900'} opacity-0 transition-opacity peer-checked:opacity-100 font-bold`} strokeWidth={4} />
                      </div>
                      <span className={`text-xs font-medium transition-colors ${activeTheme === 'midnight-black' ? 'text-slate-400 group-hover/chk:text-white' : 'text-slate-500 group-hover/chk:text-slate-800'}`}>Remember me</span>
                  </label>
                  <Link to={RoutePath.FORGOT_PASSWORD} className={`text-xs font-semibold transition-colors hover:underline decoration-2 underline-offset-2 ${activeTheme === 'midnight-black' ? 'text-[#F7C948] hover:text-[#D4A017]' : 'text-slate-800 hover:text-black'}`}>Forgot password?</Link>
              </div>
            ...
            {/* Divider */}
             ...
             <p className={`mt-8 text-[13px] font-medium ${activeTheme === 'midnight-black' ? 'text-slate-400' : 'text-slate-500'}`}>
               Don't have an account? <Link to={RoutePath.SIGNUP} className={`font-bold transition-colors hover:underline decoration-2 underline-offset-2 ${activeTheme === 'midnight-black' ? 'text-[#F7C948] hover:text-[#D4A017]' : 'text-slate-800 hover:text-black'}`}>Sign up</Link>
             </p>
        ...
        <div className={`absolute top-10 left-0 w-2 h-2 rounded-full blur-[1px] animate-ping ${activeTheme === 'midnight-black' ? 'bg-[#F7C948]/20' : 'bg-slate-500/10'}`} />
        <div className={`absolute bottom-10 right-0 w-3 h-3 rounded-full blur-[2px] animate-bounce ${activeTheme === 'midnight-black' ? 'bg-white/20' : 'bg-slate-500/10'}`} style={{ animationDuration: '3s' }} />
